...
<main class="rest">
    <div class="container">
        ...
        <div class="infoApi">
            <?php
            if ($aVistaRest['apiAEMET'] != null && $_SESSION['municipioSeleccionado'] != null) {
                echo('<p class="mensajeAEMET">Según la <a href="' . $aVistaRest['apiAEMET']['origen']['enlace'] . '" target="_blank">AEMET</a>, el pronóstico de hoy de <u>' . $aVistaRest['apiAEMET']['nombre'] . '</u>, de la provincia de <u>' . $aVistaRest['apiAEMET']['provincia'] . '</u>, es el siguiente:</p>');
                echo('<table class="tablaAEMET">');
                echo('<tr><th>Hora</th><th>Temperatura</th></tr>');
                for ($i = 0; $i < 24; $i++) {
                    echo('<tr>');
                    echo('<td>' . $aVistaRest['apiAEMET']['prediccion']['dia'][1]['temperatura'][$i]['periodo'] . ':00</td>');
                    echo('<td>' . $aVistaRest['apiAEMET']['prediccion']['dia'][1]['temperatura'][$i]['value'] . 'ºC</td>');
                    echo('</tr>');
                }
                echo('</table>');
            } else {
    ...
    <div class="wrapper">
        ...
        <div class="infoApiHoroscopo">
            <?php
            if ($aVistaRest['apiHoroscopo'] != null) {
                echo($aVistaRest['apiHoroscopo']);
            } else {
