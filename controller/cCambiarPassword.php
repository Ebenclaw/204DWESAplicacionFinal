...
if (isset($_REQUEST['salirCambiarContraseña'])) {
    $_SESSION['paginaAnterior'] = 'cambiarContraseña';
...
if (isset($_REQUEST['confirmarCambios'])) { 
    ...
    if (!UsuarioPDO::validarUsuario($_SESSION['user204DWESLoginLogout']->getCodUsuario(), $_REQUEST['contraseñaActual'])) {
...
if ($entradaOK) { // Si el usuario ha rellenado el formulario correctamente rellenamos el array aFormulario con las respuestas introducidas por el usuario
     ...
     */ 
    $_SESSION['user204DWESLoginLogout'] = UsuarioPDO::cambiarPassword($_SESSION['user204DWESLoginLogout'], $_REQUEST['nuevaContraseña']);
    $_SESSION['paginaAnterior'] = 'cambiarContraseña';
    $_SESSION['paginaActiva'] = 'miCuenta'; // Asigno a la página en curso la pagina de miCuenta
    header('Location: index.php'); // Redirecciono al index de la APP
    exit;
